...
const BleyaAIAnimatedLanding = () => {
  ...
  useEffect(() => {
    const animateWords = () => {
      ...
      wordElements.forEach((word) => {
        const delay = Number.parseInt(word.getAttribute("data-delay")) || 0
        setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = (e) => {
      if (e.target) e.target.style.textShadow = "0 0 20px rgba(96, 165, 250, 0.5)"
    }
      ...
      if (!scrolled) {
        ...
        floatingElementsRef.current.forEach((el, index) => {
          setTimeout(
            ...
            Number.parseFloat(el.style.animationDelay || "0") * 1000 + index * 100,
          )
        })
      }
    }
  ...
  const pageStyles = `
    ...
    .word-animate:hover { color: #60a5fa; transform: translateY(-2px); }
    .grid-line { stroke: #3b82f6; stroke-width: 0.5; opacity: 0; stroke-dasharray: 5 5; stroke-dashoffset: 1000; animation: grid-draw 2s ease-out forwards; }
    .detail-dot { fill: #60a5fa; opacity: 0; animation: pulse-glow 3s ease-in-out infinite; }
    .corner-element-animate { position: absolute; width: 40px; height: 40px; border: 1px solid rgba(96, 165, 250, 0.2); opacity: 0; animation: word-appear 1s ease-out forwards; }
    .text-decoration-animate { position: relative; }
    .text-decoration-animate::after { content: ''; position: absolute; bottom: -4px; left: 0; width: 0; height: 1px; background: linear-gradient(90deg, transparent, #60a5fa, transparent); animation: underline-grow 2s ease-out forwards; animation-delay: 2s; }
    @keyframes underline-grow { to { width: 100%; } }
    .floating-element-animate { position: absolute; width: 2px; height: 2px; background: #60a5fa; border-radius: 50%; opacity: 0; animation: float 4s ease-in-out infinite; animation-play-state: paused; }
    ...
    .ripple-effect { position: fixed; width: 4px; height: 4px; background: rgba(96, 165, 250, 0.6); border-radius: 50%; transform: translate(-50%, -50%); pointer-events: none; animation: pulse-glow 1s ease-out forwards; z-index: 9999; }
  `

  return (
    <>
      <style>{pageStyles}</style>
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-black to-blue-950 text-slate-100 font-sans overflow-hidden relative">
        <svg
        ...
        >
          <div className="absolute top-0 left-0 w-2 h-2 bg-blue-400 opacity-30 rounded-full"></div>
        </div>
        <div
          className="corner-element-animate top-4 right-4 sm:top-6 sm:right-6 md:top-8 md:right-8"
          style={{ animationDelay: "4.2s" }}
        >
          <div className="absolute top-0 right-0 w-2 h-2 bg-blue-400 opacity-30 rounded-full"></div>
        </div>
        <div
          className="corner-element-animate bottom-4 left-4 sm:bottom-6 sm:left-6 md:bottom-8 md:left-8"
          style={{ animationDelay: "4.4s" }}
        >
          <div className="absolute bottom-0 left-0 w-2 h-2 bg-blue-400 opacity-30 rounded-full"></div>
        </div>
        <div
          className="corner-element-animate bottom-4 right-4 sm:bottom-6 sm:right-6 md:bottom-8 md:right-8"
          style={{ animationDelay: "4.6s" }}
        >
          <div className="absolute bottom-0 right-0 w-2 h-2 bg-blue-400 opacity-30 rounded-full"></div>
        </div>
        ...
        <div className="relative z-10 min-h-screen flex flex-col justify-between items-center px-6 py-10 sm:px-8 sm:py-12 md:px-16 md:py-20">
          <div className="text-center">
            <h2 className="text-xs sm:text-sm font-mono font-light text-blue-300 uppercase tracking-[0.2em] opacity-80">
            ...
            <div className="mt-4 w-12 sm:w-16 h-px bg-gradient-to-r from-transparent via-blue-400 to-transparent opacity-30 mx-auto"></div>
          </div>
          ...
          <div className="text-center max-w-5xl mx-auto relative">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extralight leading-tight tracking-tight text-slate-50 text-decoration-animate">
              ...
              <div className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-thin text-blue-200 leading-relaxed tracking-wide">
            ...
            <div
              className="absolute -left-6 sm:-left-8 top-1/2 transform -translate-y-1/2 w-3 sm:w-4 h-px bg-blue-400 opacity-0"
              style={{ animation: "word-appear 1s ease-out forwards", animationDelay: "3.2s" }}
            ></div>
            <div
              className="absolute -right-6 sm:-right-8 top-1/2 transform -translate-y-1/2 w-3 sm:w-4 h-px bg-blue-400 opacity-0"
              style={{ animation: "word-appear 1s ease-out forwards", animationDelay: "3.4s" }}
            ></div>
          </div>

          <div className="text-center">
            <div className="mb-4 w-12 sm:w-16 h-px bg-gradient-to-r from-transparent via-blue-400 to-transparent opacity-30 mx-auto"></div>
            <h2 className="text-xs sm:text-sm font-mono font-light text-blue-300 uppercase tracking-[0.2em] opacity-80">
...
export default BleyaAIAnimatedLanding
